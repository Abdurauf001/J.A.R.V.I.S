...
def _classify_runtime_error(exc: BaseException) -> tuple[str, str | None]:
    ...
    if "api key not valid" in text or "invalid api key" in text:
        return (
            "fatal_api_key",
            "ERR: Vision session stopped. Gemini API key invalid.",
        )
    ...
    if (
        ...
        return (
            "retry_network",
            "ERR: Vision session network/DNS muammosi sabab ulanmayapti.",
        )
    ...
    if any(pattern in text for pattern in audio_patterns):
        return (
            "retry_audio",
            "ERR: Vision audio output device topilmadi yoki band.",
        )
    ...
    async def _session_loop(self) -> None:
        ...
        while True:
            try:
                ...
                kind, message = _classify_runtime_error(exc)
                if False:
                    print(f"[Vision] ⚠️  Session error: {exc}")
                print(f"[Vision] Session error: {_collect_exception_text(exc)}")
                if message:
                    print(f"[Vision] {message}")
                    if self._player:
                        self._player.write_log(message)
                if kind == "fatal_api_key":
                    return
            finally:
                self._session = None
