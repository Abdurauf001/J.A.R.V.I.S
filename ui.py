...
class JarvisUI:
    ...
    def _api_keys_exist(self) -> bool:
        ...
        try:
            ...
            return (
                self._looks_like_gemini_api_key(data.get("gemini_api_key", ""))
                and bool(data.get("os_system"))
            )
        except Exception:
            return False
    ...
    def _save_api_keys(self):
        ...
        if not self._looks_like_gemini_api_key(gemini):
            self.gemini_entry.configure(highlightthickness=1,
                                        highlightbackground=C_RED,
                                        highlightcolor=C_RED)
            self.write_log("ERR: Enter a valid Gemini API key. It usually starts with 'AIza'.")
            return
